...
export default function getThread({
	...
	const getThread = () => {
		function getThread() {
			...
			return request(api.getThread, parameters).then((response) => {
				return {
					response
				}
			})
	...
	const getArchivedThread = () => {
		return request(api.getArchivedThread, parameters).then((response) => {
			return {
				archived: true,
				response
			}
	...
	const getThreadFromArchiveMakaba = () => {
		return request(api.getArchivedThread, { returnResponseInfoObject: true }, parameters).then(
			({ response, url }) => {
				let archivedAt
