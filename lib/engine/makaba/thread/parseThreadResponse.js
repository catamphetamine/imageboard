...
export default function parseThreadResponse(response, options) {
	...
	const board = response.board && parseBoard(response.board, options)
	...
	return {
		// * There is `response.board` in "get thread" API response.
		// * There is `response.board` in "get threads list" API response.
		// * There is `response.board` in "get threads list page" API response.
		// * There is no `response.board` in "incremental thread update" API response.
		board,
		thread
	}
}
