...
export default function parseAttachment(file, { transformAttachmentUrl }) {
	const formatUrl = (url) => {
	...
	// NSFW flag (isn't present on all boards).
	// if (file.nsfw) {
	//   ...
	// }
...
function parsePicture(file, mimeType, name, formatUrl) {
	...
	if (file.type === STICKER_FILE_TYPE) {
		...
		// * `file.pack` — Sticker pack ID.
		// * `file.sticker` — Sticker ID.
		// * `file.install` — Sticker pack installation URL.
	}
	return attachment
}
