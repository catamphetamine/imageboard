...
import parseCreateThreadResponseViChan from '../vichan/post/parseCreateThreadResponse.js'
...
import parseCreateCommentResponseViChan from '../vichan/post/parseCreateCommentResponse.js'
...
import parseReportCommentResponseInfinity from '../infinity/report/parseReportCommentResponseHtml.js'
...
export default class FourChan extends Engine {
	...
	parseThread(response, options) {
		const { thread: threadData } = parseThreadResponse(response, options)
		const thread = this.createThreadObject(threadData, options)
		...
		//
		if (
			this.options.engine === 'OpenIB' ||
			this.options.engine === 'vichan' ||
			this.options.engine === 'lainchan'
		) {
	...
	parseCreateThreadResponse(response) {
		switch (this.options.engine) {
			...
			case 'vichan':
			case 'lainchan':
				return parseCreateCommentResponseViChan(response)
			default:
	...
	parseReportCommentResponse(response) {
		switch (this.options.engine) {
			case 'infinity':
			case 'OpenIB':
				return parseReportCommentResponseInfinity(response)
			default:
