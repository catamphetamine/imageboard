...
export default function parseComment(post, {
	...
	if (content) {
		...
		// 		authorBanXxx = match[0]
		// 		content = content.replace(LAINCHAN_POST_WARNING_REGEXP, '')
		// 	}
		// }
	}
	const authorRole = parseAuthorRole(post, { chan, boardId, capcodes })
	...
	const comment = {
		...
		title: post.sub && unescapeText(post.sub),
		content,
	...
	if (post.board_flag) {
		comment.authorIconUrl = authorIconUrl
			.replace('{boardId}', boardId)
			.replace('{authorIconId}', post.board_flag.toLowerCase())
		comment.authorIconName = post.flag_name
	}
	return comment
}
