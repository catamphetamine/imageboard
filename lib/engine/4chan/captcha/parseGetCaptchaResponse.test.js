import parseGetCaptchaResponse from './parseGetCaptchaResponse.js'

describe('engine/4chan/captcha/parseGetCaptchaResponse', function() {
	it('should parse "get CAPTCHA" response', function() {
		const timestampBefore = Date.now()

		const expiresIn = 120
		const canRequestNewCaptchaIn = 4

		const { captcha, canRequestNewCaptchaAt } = parseGetCaptchaResponse({
			challenge: 'bvPlMuE0CKUJ72T8.e466e2a19c9fdfe555def2baf3f00452aff19230758abc352aa84f971f19d9d0',
			ttl: expiresIn,
			cd: canRequestNewCaptchaIn,
			img: '',
			img_width: 281,
			img_height: 80,
			bg: '',
			bg_width: 330
		})

		const timestampAfter = Date.now()

		expect(canRequestNewCaptchaAt.getTime() >= timestampBefore + canRequestNewCaptchaIn * 1000).to.equal(true)
		expect(canRequestNewCaptchaAt.getTime() <= timestampAfter + canRequestNewCaptchaIn * 1000).to.equal(true)

		expect(captcha.expiresAt.getTime() >= timestampBefore + expiresIn * 1000).to.equal(true)
		expect(captcha.expiresAt.getTime() <= timestampAfter + expiresIn * 1000).to.equal(true)

		delete captcha.expiresAt

		captcha.should.deep.equal({
			id: 'bvPlMuE0CKUJ72T8.e466e2a19c9fdfe555def2baf3f00452aff19230758abc352aa84f971f19d9d0',
			type: 'text',
			challengeType: 'image-slider',
			image: {
				type: 'image/png',
				url: 'data:image/png;base64,' + '',
				width: 281,
				height: 80
			},
			backgroundImage: {
				type: 'image/png',
				url: 'data:image/png;base64,' + '',
				width: 330,
				height: 80
			}
		})
	})

	it('should throw a refresh limit exceeded error', function() {
		expect(() => {
			parseGetCaptchaResponse({
				error: 'You have to wait a while before doing this again',
				cd: 123
			}, {})
		}).to.throw('RATE_LIMIT_EXCEEDED')
	})
})