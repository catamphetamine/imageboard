...
export default function parseBoard(board) {
	...
	if (board.board_flags) {
		parsedBoard.features.badges = true
	...
	if (board.country_flags) {
		...
		parsedBoard.features.countryFlags = true
	}
	if (board.user_ids) {
	...
	parsedBoard.commentContentMaxLength = board.max_comment_chars
	// `4chan` only allows a single attachment in a comment.
	// Hence the single attachment max size is the total attachments max size.
	parsedBoard.attachmentMaxSize = board.max_filesize // in bytes
	parsedBoard.videoAttachmentMaxSize = board.max_webm_filesize // in bytes
	parsedBoard.videoAttachmentMaxDuration = board.max_webm_duration // in seconds
	parsedBoard.createThreadMinInterval = board.cooldowns.threads
