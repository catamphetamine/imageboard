...
export default function parseComment(post, {
	...
	const authorRole = parseAuthorRole(post, { capcodes })
	...
	const comment = {
		...
		title: post.subject && unescapeText(post.subject),
		content,
		...
		authorBan: post.banMessage ? {
			reason: post.banMessage // '(USER WAS BANNED FOR THIS POST)'
		} : undefined,
