...
export default function getThreads({
	...
	return makeHttpRequests(requests).then(
		(results) => {
			...
			let threadsList = parseThreads(threadsResult.value.response, {
				...parameters,
				...restOptions,
				withLatestComments,
			...
			for (const threadsWithLatestCommentsPageResult of threadsWithLatestCommentsPageResults) {
				...
				// Move all non-main comments to `latestComments` array.
				// It has to be done here instead of in `parseThreadsPageResponse.js`
				// because if it was done in `parseThreadsPageResponse.js`,
...
function getThreadsListWithLatestCommentsFromThreadsListAndThreadPages(threadsList, threadsPages) {
	...
	return firstPage.concat(threads)
}
