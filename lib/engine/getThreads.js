...
export default function getThreads({
	...
	const promises = [request(apiMethod, parameters)]

	// Fetch threads rating.
	if (sortByRating && api.getThreadsStats) {
		promises.push(request(api.getThreadsStats, parameters))
	}
	...
	if (fetchPages) {
		...
		while (i < maxPagesToFetch) {
			...
			promises.push(request(apiMethod, {
				...parameters,
				pageIndex: i,
				page: i + 1
			}))
