...
export default function generateThreadTitle(thread, options = {}) {
	const {
		...
		// See the "Messages" section in the readme.
		messages,
		// (optional)
	...
	if (!thread.title) {
		const autogeneratedTitle = generatePostQuote(thread.comments[0], {
			messages: messages && {
		...
		// Don't assign `autogeneratedTitle: undefined` property
		// just so it looks a bit cleaner in tests when comparing
		// expected and actual thread object.
		if (autogeneratedTitle !== undefined) {
			thread.autogeneratedTitle = autogeneratedTitle
		}
	}
}