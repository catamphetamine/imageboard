...
export default function supportsFeature(idOrConfig, feature) {
	...
	switch (feature) {
		...
		/*
		 * Returns `true` if the imageboard can return a list of threads with "latest comments".
		 */
		case 'getThread.withLatestComments':
			return Boolean(imageboardConfig.api && ((
				imageboardConfig.api.getThreads &&
				imageboardConfig.api.getThreads.features &&
				imageboardConfig.api.getThreads.features.withLatestComments
			) || (
				imageboardConfig.api.getThreadsWithLatestCommentsPage
			)))
		...
		/*
		 * Returns `true` if the imageboard supports "rate comment" API.
		 */
		case 'rateComment':
			return Boolean(imageboardConfig.api && (
				imageboardConfig.api.rate
			))
