...
export default class Engine {
	...
	getCaptcha(params) {
		...
		return this.sendRequest({
			...this.options.api.getCaptcha,
			returnResponseInfoObject: true
		}, params).then(
			({ response, headers }) => this.parseGetCaptchaResponse(response, {
				headers,
				getSetCookieHeaders: this.options.getSetCookieHeaders,
				captchaExpiresAtCookieName: this.options.captchaExpiresAtCookieName,
				captchaExpiresAtCookieFormat: this.options.captchaExpiresAtCookieFormat,
				captchaIdCookieName: this.options.captchaIdCookieName,
	 ...
	 */
	solveCaptcha(params) {
		if (!this.parseSolveCaptchaResponse) {
			throw new Error('Not implemented for this engine')
		}
		return this.sendRequest(this.options.api.solveCaptcha, params).then(
			(response) => this.parseSolveCaptchaResponse(response)
		)
	}
	 ...
	 */
	createBoard(params) {
		if (!this.parseCreateBoardResponse) {
			throw new Error('Not implemented for this engine')
		}
		return this.sendRequest(this.options.api.createBoard, params).then(
			(response) => this.parseCreateBoardResponse(response)
		)
	}
	 ...
	 */
	deleteBoard(params) {
		if (!this.parseDeleteBoardResponse) {
			throw new Error('Not implemented for this engine')
		}
		return this.sendRequest(this.options.api.createBoard, params).then(
			(response) => this.parseDeleteBoardResponse(response)
		)
	}
	...
	reportComment(params) {
		if (!this.parseReportCommentResponse) {
			throw new Error('Not implemented for this engine')
		}
		return this.sendRequest(this.options.api.report, params).then(
			(response) => this.parseReportCommentResponse(response)
		)
	}
	 ...
	 */
	renewBlockBypass(params) {
		if (!this.parseGetBlockBypassResponse) {
			throw new Error('Not implemented for this engine')
		}
		return this.sendRequest(this.options.api.getBlockBypass, params).then(
			(response) => this.parseGetBlockBypassResponse(response)
		)
	}
	 ...
	 */
	renewBlockBypass(params) {
		if (!this.parseRenewBlockBypassResponse) {
			throw new Error('Not implemented for this engine')
		}
		return this.sendRequest(this.options.api.renewBlockBypass, params).then(
			(response) => this.parseRenewBlockBypassResponse(response)
		)
	}
	 ...
	 */
	renewBlockBypass(params) {
		if (!this.parseValidateBlockBypassResponse) {
			throw new Error('Not implemented for this engine')
		}
		return this.sendRequest(this.options.api.validateBlockBypass, params).then(
			(response) => this.parseValidateBlockBypassResponse(response)
		)
	}
...
function parseResponse({ responseText, requestHeaders, url, error }) {
	if (!responseText) {
		return
	}
