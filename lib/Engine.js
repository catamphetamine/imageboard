...
export default class Engine {
	constructor(chanSettings, {
		...
		if (!engineSettings) {
			throw new Error('`engineSettings` parameter not passed by "' + chanSettings.engine + '" engine constructor')
		}

		// Merge imageboard custom settings with the engine's settings.
		chanSettings = mergeConfigs(engineSettings, chanSettings)

		// Apply any `api` overrides.
		if (chanSettings.apiOverride) {
			chanSettings.api = merge({}, chanSettings.api, chanSettings.apiOverride)
			delete chanSettings.apiOverride
		}
	 ...
	 * @param  {object} parameters — `{ boardId, authorName?, authorEmail?, title?, content?, attachments?, attachmentSpoilers?, attachmentFileTags?, textOnly?, accessToken?, captchaId?, captchaSolution? }`.
	 ...
	 * @param  {object} parameters — `{ boardId, threadId, authorName?, authorEmail?, title?, content?, attachments?, attachmentSpoilers?, attachmentFileTags?, textOnly?, accessToken?, captchaId?, captchaSolution? }`.
	 * @return {Promise<number>} Returns new thread ID or new comment ID. Throws an error in case of an error. If the error is "banned" then the error may have properties: `banId`, `banReason`, `banBoardId`, `banEndsAt`.
	 */
		...
		switch (feature) {
			...
			case 'getThreads.sortByRating':
				return Boolean(this.options.api && ((
					...
					this.options.api.getThreads.features.rating
				) || (
					this.options.api.getThreadsStats &&
					this.options.api.getThreadsStats.features &&
					this.options.api.getThreadsStats.features.rating
				)))
			default:
